    ...
    public GameObject getDisk(){
        ...
        if(free.Count>0){
            ...
            new Vector3(Random.Range(-12,12),7,0), Quaternion.identity, null) as GameObject;
                a.GetComponent<DickAction>().speed=(int)Random.Range(1,6);
                a.GetComponent<DickAction>().score=a.GetComponent<DickAction>().speed;
            }
            else{
                a = Instantiate(Resources.Load("Plant2"),
            new Vector3(Random.Range(-12,12),7,0), Quaternion.identity, null) as GameObject;
                a.GetComponent<DickAction>().speed=(int)Random.Range(1,6);
                a.GetComponent<DickAction>().score=a.GetComponent<DickAction>().speed*2;
            }
        }
        used.Add(a);
        return a;
